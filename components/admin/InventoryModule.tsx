...
export function InventoryModule({
  ...
  // Estado para ventas rápidas desde inventario / checkout
  ...
  return (
    <div className="space-y-6 text-slate-900">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 leading-tight">
              {isAdmin ? "Control de Inventario y Productos" : "Consulta y Ventas de Inventario"}
            </h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              {isAdmin
                ? "Gestión precisa de variantes, tallas, stock, precios y colores."
                : "Consulta rápidamente prendas disponibles, colores y precios para realizar cobros."}
            </p>
          </div>
        </div>
        <button
      ...
      {/* Layout adaptable dinámicamente según rol */}
      <div className={`grid grid-cols-1 ${isAdmin ? "lg:grid-cols-12" : "grid-cols-1"} gap-6 items-start`}>
        ...
        {/* TABLA DERECHA (TOMA 7 COLS SI ES ADMIN, 12 COLS SI ES CAJERO) */}
        <div className={`${isAdmin ? "lg:col-span-7" : "col-span-1"} bg-white border border-slate-300 rounded-2xl overflow-hidden shadow-sm`}>
          <div className="bg-slate-100 border-b border-slate-200 px-4 py-3 flex justify-between items-center">
            <h3 className="font-bold text-slate-900 text-sm">
              {isAdmin ? "Productos Registrados" : "Catalogo de Prendas y Stock"}
            </h3>
                        ...
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              <p className="font-bold text-slate-900 truncate max-w-[120px] sm:max-w-[180px]">
                                {product.name}
                              </p>
                        ...
                        <td className="py-2.5 px-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              ...
                              className="p-1.5 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 rounded-lg transition-colors flex items-center gap-1 font-semibold text-xs"
                              title="Registrar venta directa"
                            >
                              <ShoppingCart size={15} />
                              {!isAdmin && <span className="hidden sm:inline">Vender</span>}
                            </button>
                            {isAdmin && (
                              <>
                                <button
