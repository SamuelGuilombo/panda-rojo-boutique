...
export default function AdminLayout({
  ...
  const { isAuthenticated, checkingAuth, isAdmin, isCashier, user, logout } = useAuth()
  ...
  // Nombres más concisos para evitar la saturación en la barra superior
  const allNavItems = [
    ...
    <div className="fixed inset-0 z-50 bg-neutral-950 text-neutral-100 flex flex-col font-sans overflow-y-auto">
      ...
      <header className="sticky top-0 z-50 border-b border-neutral-800 bg-neutral-950/95 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 flex items-center justify-between h-16 gap-2">
          ...
          <nav className="hidden md:flex items-center gap-1 overflow-x-auto py-1 no-scrollbar">
            {navItems.map((item) => {
          ...
          <div className="flex items-center gap-2 shrink-0">
            {user?.name && (
              <div className="hidden xl:flex items-center gap-1.5 text-xs text-neutral-400 bg-neutral-900 px-2.5 py-1 rounded-md border border-neutral-800">
                ...
                <span className="font-medium text-neutral-300 max-w-[110px] truncate">{user.name}</span>
              </div>
            )}
